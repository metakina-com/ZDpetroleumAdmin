<template>
  <div class="user">
    <div class="filter-container" style="padding-bottom: 30px">
      ...
      <el-date-picker value-format="yyyy-MM-dd" v-model="listQuery.startTimes" type="datetime" placeholder="开始时间">
      </el-date-picker>
      <el-date-picker value-format="yyyy-MM-dd" v-model="listQuery.endTimes" type="datetime" placeholder="结束时间">
      </el-date-picker>
    ...
    <el-row :gutter="21" style="padding: 0">
      <el-col :span="3">
      ...
      <el-col :span="3">
        <div class="grid-content bg-purple">
          <span>伞下总返佣</span>
          <span>{{ sumData.sumReturncommission || "--" }}</span>
        </div>
      </el-col>
      <el-col :span="3">
        <div class="grid-content bg-purple">
    ...
    <el-tree :data="data" :props="defaultProps" node-key="id" :expand-on-click-node="false"
      @node-click="handleNodeClick" :highlight-current="true" :default-expand-all="true">
      <div class="custom-tree-node" slot-scope="{ node, data }" style="width: 100%">
        <div style="
          ...
          ">
          <span>
            <span style="margin: 0 10px">邮箱：{{ data.mail }}</span>
            ...
            <span style="margin: 0 10px">总资产： {{ data.sumZzcCount}}</span>
            <span style="margin: 0 10px">总返佣： {{ data.sumContractCommInfo}}</span>
          </span>
    ...
    <el-dialog :visible.sync="modifyShow">
      <el-form :data="modifyData" label-position="left" label-width="100px" style="width: 300px; margin-left: 50px">
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        // if(this.listQuery.mail !== ""){
        //   // console.log(this.listQuery.mail)
        //   var mail = this.listQuery.mail;
        //   console.log(mail,1222122)
        ...
        this.$http
          ...
          .then((res) => {
            ...
            //   obj.sumZzcCount = item.sumZzcCount; //总资产
            //   this.data.push(obj);
            //   // console.log(obj)
            // });
      ...
      //   this.$http
      //     .get("/admin/sysAdmin/zkMembers", {
      //       mail: data.label
      //     })
      //     .then((res) => {
      //       // console.log(res.result.welMembers);
      //       var xiaji = "";
      //       xiaji = res.result.welMembers;
      //       // console.log(xiaji.children)
      //       if (res.result.welMembers) {
      //         data.children = [];
      //         res.result.welMembers.forEach((item) => {
      //           console.log(item)
      //           // console.log(item.children[0].mail)
      //           const newChild = {
      //             id: item.id,
      //             label: item.mail,
      ...
      //             sumFee: item.sumFee,
      //             // sumZzcCount: item.sumZzcCount,//总资产
      //             children: [],
      //           };
      //           if (!data.children) {
