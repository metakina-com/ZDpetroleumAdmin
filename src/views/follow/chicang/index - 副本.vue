...
export default {
  ...
  methods: {
    getList() {
      ...
      this.$http
        .get("/admin/sysAdmin/getFollowCreateOrders", this.listQuery)
        .then((res) => {
          if (res.status == "SUCCEED") {
            this.list = res.result.records;
            // this.list.forEach((element) => {
            //   element.createTime = this.getDatetime(element.createTime / 1000);
            // });
            this.list.forEach((element) => {
              element.matchTime = this.getDatetime(element.updateTime);
            });
