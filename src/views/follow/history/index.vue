...
<el-dialog :visible.sync="modifyShow" width="1000px">
        <!-- <span>{{ this.YKbalanceList }}</span> -->
        <div v-for="(item,index)  in YKbalanceList" :key="index">
        <h5>{{item}}</h5>
      </div>
...
export default {
  ...
  methods: {
    ...
    detail(row) {
      ...
      this.YKbalanceList = row.row.balanceList;
    ...
    getList() {
      ...
      this.$http
        .get("/admin/sysAdmin/getFollowContractHistory", this.listQuery)
        .then((res) => {
