  ...
  <div class="user">
    ...
    <el-table :data="list" border tooltip-effect="dark" style="width: 100%">
     ...
     <template slot-scope="row">
          <p  v-if="row.row.address">{{row.row.address}}</p>
          <p v-else>{{row.row.wallet}}</p>
        </template>
      </el-table-column>
       <!-- <el-table-column
        v-if="wallet"
      ...
      <template slot-scope="row">
        ...
        <p style="color: red;" v-if="row.row.state == 'REJECT'">驳回</p>
          <p style="color: green;" v-if="row.row.result == 'true'">审核通过</p>
          <p style="color: red;" v-if="row.row.result == 'false'">驳回</p>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      ...
      // 获取用户管理
      // admin/sysAdmin/getExtractCoinHistory
      this.listQuery.member = window.localStorage.getItem("userId");
      this.$http
        .get("/admin/sysAdmin/getExtractCoinHistory", this.listQuery)
        .then((res) => {
